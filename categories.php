    ...
    <div class="row">
      ...
      <div class="col-lg-9">
        ...
        <div class="row">
          ...
          $stmt->execute();
          // $stmt = $db->query('SELECT * FROM posts WHERE post_category = :categoryName');
          $totalPages = ceil($stmt->rowCount() / $limit);
          // Fetch Categories
          $query = 'SELECT * FROM posts WHERE post_category = :categoryName ORDER BY post_id DESC LIMIT ' . $start . ', ' . $limit;
          $stmt = $db->prepare($query);
          $stmt->bindParam(':categoryName', $categoryName);
          $stmt->execute();
        ...
        <ul class="pagination d-flex justify-content-center mt-5">
          <li class="page-item <?php if($page - 1 == 0) echo 'disabled'; ?>">
            <a class="page-link" href="?category=<?php echo $categoryName ?>&page=<?php echo $page - 1; ?>">السابق</a>
          </li>
          <?php
          ...
          for($i = 1; $i <= $totalPages; $i++) {
            ...
            <li class="page-item <?php if($page == $i) echo 'active'; ?>">
              <a class="page-link"
                 href="?category=<?php echo $categoryName ?>&page=<?php echo $i; ?>"><?php echo $i; ?></a>
            </li>
            <?php
          }
          ?>
          <li class="page-item <?php if($page + 1 > $totalPages) echo 'disabled'; ?>">
            <a class="page-link" href="?category=<?php echo $categoryName ?>&page=<?php echo $page + 1; ?>">التالي</a>
          </li>
        </ul>
      </div>
      <!-- Sidebar Column -->
